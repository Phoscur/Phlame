...
/**
 * Phorge — the forge (PLAN-CONTAINERS O0): orchestration MCP for the containerized
 * runners. A closed verb vocabulary over compose.test.yml; the game sandbox lives in
 * its own server (phlame-game, tools/mcp). stdio transport — stdout is the protocol
 * channel, so never console.log here; diagnostics go to stderr.
 */
const server = new McpServer({ name: 'phorge', version: '0.1.0' });
