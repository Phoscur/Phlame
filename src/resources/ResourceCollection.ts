...
export default class ResourceCollection<Types extends ResourceIdentifier> {
  ...
  createByType<Type extends Types>(type: Type, amount = 0): Resource<Type>|Energy<Type> {
  ...
  getByType<Type extends Types>(type: Type): Resource<Type>|Energy<Type>|undefined {
  ...
  map<GenericReturn>(iterator: (resource: ResourceLike<Types>, type: Types) => GenericReturn): GenericReturn[] {
    return this.types.reduce<GenericReturn[]>((entries: GenericReturn[], type: Types) => {
      const entry: Resource<Types>|Energy<Types>|undefined = this.entries[type];
